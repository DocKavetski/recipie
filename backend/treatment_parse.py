...
# Старт схемы приёма. Важно: «по1/4т» не должно цепляться за «4т»
# (теряется «по», схема начинается с «4»).
_SCHEME_HINT = re.compile(
    ...
    r"табл|1[,.]5\s*т"
    r")\b|"
    r"(?<![\d/])\d+\s*/\s*\d+|"  # 1/4, 1/2 без «по»
    r"[½⅓⅔¼¾]|"
    r"(?<!/)\b\d+\s*т\b"  # «1т», но не знаменатель «1/4т»
    r")",
    re.IGNORECASE,
)
