...
# Короткие формулировки под бланк формы 1.
...
def _category_fallback_schemes(category: str, unit: str) -> list[str]:
    ...
    if key in {"сиозс", "сиозсн"}:
        return [f"по 1 {unit} утром", f"по 1/2 {unit} утром", f"по 1 {unit} вечером"]
    if key in {"тца", "антидепрессанты"}:
        return [f"по 1 {unit} на ночь", f"по 1/2 {unit} на ночь", f"по 1 {unit} вечером"]
    ...
    if key == "сон":
        return [f"по 1 {unit} на ночь", f"по 1/2 {unit} на ночь", f"по 1 {unit} за 30 мин. до сна"]
    if key == "ноотропы":
        return [f"по 1 {unit} утром", f"по 1 {unit} утром и вечером"]
    if key == "сопутствующие":
        return [f"по 1 {unit} утром", f"по 1/2 {unit} утром", f"по 1 {unit} 2 раза в день"]
...
def _default_schemes(item: dict[str, Any]) -> list[str]:
    ...
    if by_mnn:
        return list(by_mnn)[:6]
    unit = _form_unit(item)
    return _category_fallback_schemes(item.get("category", ""), unit)[:6]
