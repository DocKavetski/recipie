...
const addManualDrugBtn = document.getElementById("addManualDrugBtn");
const saveManualDrugBtn = document.getElementById("saveManualDrugBtn");
...
function getFormState() {
    return {
        ...
        drugs: Array.from(drugRowsContainer.querySelectorAll(".drug-row")).map(getRowState),
    };
}
...
function refreshDrugsEmptyState(container = drugRowsContainer) {
    ...
    emptyState.classList.toggle("is-visible", !container || container.children.length === 0);
}
                ...
                <div class="rx-void" title="Поле закрыто символом Ƶ — дописывать нельзя" aria-label="Пустое поле закрыто">
                    <svg class="rx-void-mark" viewBox="0 0 120 56" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
                        <g fill="none" stroke="#444" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M28 14 H92 L28 42 H92" stroke-width="1.1"></path>
                            <path d="M34 28 H86" stroke-width="0.9"></path>
                        </g>
                    </svg>
                </div>
            `;
...
function indexDailyAvailability(payload) {
    const rows = Array.isArray(payload?.rows) ? payload.rows : [];
    const byKey = {};
    for (const row of rows) {
        for (const key of [row.mnn, row.russian_name]) {
...
function cachedAvailabilityForRow(row, query = null) {
    const request = query
        ? { query, aliases: [] }
        : rowAvailabilityQuery(row);
    const names = [request.query, ...(request.aliases || [])];
    if (row) {
        names.push(
            row.querySelector(".drug-mnn-input")?.value,
            row.querySelector(".drug-russian-input")?.value,
            row.querySelector(".drug-latin-input")?.value,
        );
    }
...
function applyCachedAvailabilityToRow(row, query = null) {
    const badge = row.querySelector(".drug-availability-badge");
    const cached = cachedAvailabilityForRow(row, query);
    if (cached) {
        setAvailabilityBadge(row, cached.status || "unknown");
        if (badge) {
            badge.title = cached.message || cached.label || "Наличие на сегодня (кэш)";
            if (cached.label && cached.status !== "good" && cached.status !== "low" && cached.status !== "none") {
                badge.textContent = cached.label;
            }
        }
        return cached;
    }
    ...
    if (badge) {
        badge.textContent = dailyAvailability.checking ? "…" : "—";
        badge.title = dailyAvailability.checking
            ? "Идёт утренняя проверка наличия…"
            : "Наличие проверяется раз в день при первом запуске";
    }
    return null;
}

function applyCachedAvailabilityToAllRows() {
    Array.from(document.querySelectorAll(".drug-row")).forEach((row) => {
        applyCachedAvailabilityToRow(row);
    });
}
...
function renderDirectoryTable() {
    ...
    for (const drug of catalogDrugs) {
        ...
        const customBadge = drug.is_custom ? ' <span class="status-badge status-low">ручной</span>' : "";
...
async function refreshRowAvailability(row, query = null) {
    applyCachedAvailabilityToRow(row, query);
}

async function loadDailyAvailability({ start = false, force = false } = {}) {
    if (!window.eel) {
        return dailyAvailability;
    }
    try {
...
function addManualDrugRow(container = drugRowsContainer) {
    const row = addDrugRow(createEmptyRowData(), {
        mode: "mnn",
        availability: "none",
        dispenseQty: 30,
    }, container);
    ...
    if (badge) {
        badge.textContent = "—";
        badge.title = "Ручная строка";
    }
    setStatus("Добавлена пустая строка — заполните МНН, название, дозу и схему.");
    row.querySelector(".drug-russian-input")?.focus();
    return row;
}
...
async function bindTemplateManagerControls() {
    ...
    if (templateAddManualBtn && !templateAddManualBtn.dataset.bound) {
        templateAddManualBtn.addEventListener("click", () => {
            addManualDrugRow(templateDrugRowsContainer);
        });
        templateAddManualBtn.dataset.bound = "true";
    }
}

function bindManualDrugControls() {
    if (addManualDrugBtn && !addManualDrugBtn.dataset.bound) {
        addManualDrugBtn.addEventListener("click", () => {
            addManualDrugRow(drugRowsContainer);
            scheduleAutosave();
        });
        addManualDrugBtn.dataset.bound = "true";
    }
    ...
    if (directoryTableBody && !directoryTableBody.dataset.deleteBound) {
        directoryTableBody.addEventListener("click", async (event) => {
            const button = event.target.closest(".delete-custom-drug-btn");
            if (!button) {
                return;
            }
            const mnn = button.dataset.mnn;
            if (!mnn || !window.confirm(`Удалить ручной препарат «${mnn}» из каталога?`)) {
                return;
            }
            try {
                ...
                console.error(error);
                setStatus("Не удалось удалить препарат.");
            }
        });
        directoryTableBody.dataset.deleteBound = "true";
    }
}
